...
//                                 deleteAppointmentServerMutation.isPending
//                               }
//                               className="flex cursor-pointer items-center gap-2 px-5 py-2.5 rounded-2xl font-black text-sm transition-all shadow-lg theme-surface border theme-border theme-text hover:theme-hover-surface min-w-30 justify-center"
//                             >
//                               {isWaitingPending ? (
//                                 <CircularProgress size={16} color="inherit" />
//                               ) : (
...
//                                 deleteAppointmentServerMutation.isPending
//                               }
//                               className="flex items-center gap-2 px-5 py-2.5 rounded-2xl font-black text-sm transition-all shadow-lg theme-accent theme-text-on-accent min-w-32.5 justify-center"
//                             >
//                               {isPaidPending ? (
//                                 <CircularProgress size={16} color="inherit" />
//                               ) : (
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <button
                              onClick={() =>
                                updateToWaitingMutation.mutate(booking.uuid, {
                                  onSuccess: () =>
                                    queryClient.invalidateQueries({
                                      queryKey: ["doctorAppointments", doctor.doctorUuid, selectedDate],
                                    }),
                                })
                              }
                              disabled={isWaitingPending || updateToWaitingMutation.isPending || updateToPaidMutation.isPending || deleteAppointmentServerMutation.isPending}
                              className="flex cursor-pointer items-center gap-2 px-5 py-2.5 rounded-2xl font-black text-sm transition-all shadow-lg theme-surface border theme-border theme-text hover:theme-hover-surface min-w-30 justify-center"
                            >
                              {isWaitingPending ? (
                                <CircularProgress size={16} color="inherit" />
                          ...
                          {(isVisited && isPaid === false) && (
                            <button
                              ...
                              disabled={isPaidPending || updateToWaitingMutation.isPending || updateToPaidMutation.isPending || deleteAppointmentServerMutation.isPending}
                              className="flex items-center gap-2 px-5 py-2.5 rounded-2xl font-black text-sm transition-all shadow-lg theme-accent theme-text-on-accent min-w-32.5 justify-center"
                            >
                              {isPaidPending ? (
                                <CircularProgress size={16} color="inherit" />
                          ...
                          {(isHasBooked || isChangig) && (
                            <button
                              ...
                              disabled={isDeletePending || updateToWaitingMutation.isPending || updateToPaidMutation.isPending}
                              className="w-11 h-11 flex items-center justify-center text-red-500 hover:bg-red-50 rounded-2xl transition-all"
                              title="إلغاء وحذف الحجز من السيرفر"
                            >
                              {isDeletePending ? (
