...
//   const doctorUuid = doctor?.doctorUuid || doctor?.id;
...
//   const addAppointmentToCache = useCallback(
//     (newAppointment) => {
...
//   const updateAppointmentTimeInCache = useCallback(
//     (appointmentUuid, newTime) => {
//       queryClient.setQueryData(appointmentsQueryKey, (old) => {
//         if (!old) return old;
//         const oldAppointments = old.appointments || [];
//         return {
...
//       });
//     },
//     [appointmentsQueryKey, queryClient, targetDateISO],
//   );
...
//       return (
//         datePart === targetDateISO && formatTimeTo24Hour(timePart) === slot
//       );
//     });
...
export const useBookingModal = ({ doctor, selectedDate }) => {
  ...
  // ✅ التصحيح: استخدام doctorUuid بدل doctor.id
  const doctorUuid = doctor?.doctorUuid || doctor?.id;
  ...
  const appointmentsQueryKey = useMemo(
    () => ["doctorAppointments", doctorUuid, selectedDate],
    [doctorUuid, selectedDate],
  );

  const addAppointmentToCache = useCallback((newAppointment) => {
    queryClient.setQueryData(appointmentsQueryKey, (old) => {
      if (!old) return old;
      const oldAppointments = old.appointments || [];
      return { ...old, appointments: [...oldAppointments, newAppointment] };
  ...
  const updateAppointmentTimeInCache = useCallback((appointmentUuid, newTime) => {
    queryClient.setQueryData(appointmentsQueryKey, (old) => {
      if (!old) return old;
      const oldAppointments = old.appointments || [];
      return {
  ...
  const upsertAppointmentInCache = useCallback((appointmentData) => {
    queryClient.setQueryData(appointmentsQueryKey, (old) => {
      const oldAppointments = old?.appointments || [];
      const existingIndex = oldAppointments.findIndex(
        (app) => app.uuid === appointmentData.uuid,
  ...
  const removeAppointmentFromCache = useCallback((appointmentUuid) => {
    queryClient.setQueryData(appointmentsQueryKey, (old) => {
      if (!old) return old;
      const oldAppointments = old.appointments || [];
      return {
  ...
  useEffect(() => {
    ...
    return () => unsubscribe();
  }, [doctorUuid, targetDateISO, appointmentsQueryKey, queryClient, removeAppointmentFromCache, upsertAppointmentInCache]);
  ...
  const isSlotBusy = (slot) => {
    ...
    const apiMatch = apiAppointments.find((app) => {
      if (!app.date_time) return false;
      const [datePart, timePart] = app.date_time.replace("T", " ").split(" ");
      return datePart === targetDateISO && formatTimeTo24Hour(timePart) === slot;
    });
    const localMatch = appointments.find(
      (a) => a.doctorId === doctorUuid && a.timeSlot === slot,
    );
    return !!(apiMatch || localMatch);
  };
  ...
  const handleDrop = async (e, targetSlot) => {
    ...
    if (patientData) {
      ...
      if (patient.uuid && patient.isApiData) {
        ...
        dispatch(deleteAppointment({ doctorId: doctorUuid, timeSlot: fromSlot })); // ✅
        dispatch(
          addAppointment({ ...patient, doctorId: doctorUuid, timeSlot: targetSlot, bookingDate: displayFormattedDate }), // ✅
        );
      }
    }
  };
