...
import { ArrowBack, Email, LocalHospital, MonetizationOn, AccessTime } from "@mui/icons-material";
import { CircularProgress } from "@mui/material";
import { useDispatch } from "react-redux";
import { clearSelectedPatient } from "../../../features/patients/patientsSlice";
import { usePatientHistoryQuery } from "../../../services/patientsService";
...
const getStatusDetails = (status = "") => {
  switch (status.toLowerCase()) {
    case "has visited":
      return { text: "تمت الزيارة", classes: "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20" };
    case "is waiting":
      return { text: "في الانتظار", classes: "bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/20" };
    case "has changed":
      return { text: "تم التعديل / التأجيل", classes: "bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20" };
    default:
      return { text: "غير محدد", classes: "bg-gray-500/10 text-gray-600 dark:text-gray-400 border-gray-500/20" };
  }
};
...
const PatientRecordDetail = ({ patient, onBack }) => {
  ...
  const visits = Array.isArray(historyResponse) ? historyResponse : (historyResponse?.data || []);
      ...
      <div className="sticky -top-4 z-10 mb-6 -mx-3 px-3 py-4 backdrop-blur-md theme-surface-90 border-b theme-border -mx-2 -my-4 sm:-mx-4 md:-mx-8 lg:-mx-10">
        <div className="w-full flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-black theme-text">{patientInfo.name}</h1>
          ...
          <div className="bg-linear-to-r from-purple-500/10 to-blue-500/10 border theme-border px-4 py-2 rounded-2xl text-center sm:text-right self-start sm:self-auto">
            <p className="text-xs theme-text-muted font-bold">إجمالي المواعيد والزيارات</p>
      ...
      <div className="max-w-5xl mx-auto">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <p className="theme-text-muted text-sm animate-pulse font-bold">جاري جلب السجل الطبي المفصل للمريض...</p>
          </div>
        ) : visits.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-4">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b theme-border pb-3">
                        ...
                        <span className={`self-start sm:self-auto text-xs font-black px-4 py-1.5 rounded-full border ${statusInfo.classes}`}>
                          {statusInfo.text}
                        </span>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                        <div className="flex flex-col gap-1.5">
                          <p className="theme-text font-medium">
                            <span className="theme-text-muted">المقر والعيادة:</span> {visit.doctor_clinic}
                          </p>
                          {visit.doctor_email && (
                        ...
                        <div className="bg-linear-to-l from-gray-500/5 to-transparent p-3 rounded-2xl border theme-border flex flex-wrap items-center justify-between gap-4">
                          <div className="flex items-center gap-2">
                            <MonetizationOn className="theme-text-muted" fontSize="small" />
                            <div>
                              <p className="text-[11px] theme-text-muted font-bold">رسوم الفحص (الكشفية)</p>
                              <p className="text-sm font-black theme-text">{formatCurrency(visit.doctor_specialty?.checker)}</p>
                            </div>
                          </div>
                          <div className="h-6 w-px bg-gray-300 dark:bg-gray-700 hidden sm:block" />
                          <div>
                            <p className="text-[11px] theme-text-muted font-bold">رسوم المراجعة</p>
                            <p className="text-sm font-black theme-text-muted">{formatCurrency(visit.doctor_specialty?.review)}</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="p-3.5 rounded-2xl theme-surface-80 border theme-border flex items-center gap-3 mt-1">
                        <AccessTime className="theme-text-accent" fontSize="small" />
                        <div className="text-right">
                          <p className="text-[11px] theme-text-muted font-bold">توقيت وجدولة الموعد</p>
                          <p className="text-sm font-bold theme-text">
                            {new Date(visit.date_time.replace(/-/g, "/")).toLocaleDateString("ar-SA", {
                              weekday: "long",
                              year: "numeric",
                              month: "long",
                              day: "numeric",
          ...
          >
            <p className="theme-text-muted text-base font-medium">لا توجد مواعيد سابقة أو سجلات طبية راجعة لهذا المريض حالياً.</p>
          </Motion.div>
        )}
      </div>
    </Motion.div>
