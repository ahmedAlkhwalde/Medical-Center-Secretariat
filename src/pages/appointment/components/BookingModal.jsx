...
//   // ✅ الخانة الزمنية التي يتم إضافة حجز جديد إليها حالياً (لإظهار لودر خفيف عليها فقط)
...
//   // ✅ دالة مساعدة لتحديث الكاش محلياً (إضافة موعد جديد) بدون أي refetch
...
//       return {
//         ...old,
//         appointments: [...oldAppointments, newAppointment],
//       };
//     });
//   };

//   // ✅ دالة مساعدة لتحديث وقت موعد موجود محلياً (نقل/سحب) بدون أي refetch
...
//           return {
//             ...app,
//             date_time: `${datePart || targetDateISO} ${newTime}:00`,
//           };
//         }),
//       };
//     });
...
//     const existingApiAppointments = apiData?.appointments || [];
//     const targetApiMatch = existingApiAppointments.find((app) => {
//       if (!app.date_time) return false;
//       const [datePart, timePart] = app.date_time.replace("T", " ").split(" ");
//       return (
//         datePart === targetDateISO && timePart.substring(0, 5) === targetSlot
//       );
//     });
//     const existingLocalBooking = appointments.find(
//       (a) => a.doctorId === doctor.id && a.timeSlot === targetSlot,
//     );

//     if (targetApiMatch || existingLocalBooking) {
//       dispatch(
//         showSnackbar({
...
//       // ✅ تفعيل اللودر الخفيف على هذه البطاقة فقط
//       setPendingNewSlot(targetSlot);

//       // ✅ استخدام mutate بدلاً من mutateAsync، وتحديث الكاش يدوياً دون إعادة تحميل كامل القائمة
...
//         {
//           onSuccess: (res) => {
//             // ✅ نحاول استخراج بيانات الموعد الجديد من رد السيرفر
...
//               // تحديث الكاش محلياً فقط — بدون invalidate/refetch
//               addAppointmentToCache(newAppointment);
//             } else {
//               // كحل أخير إن لم يرجع السيرفر uuid، نضطر لإعادة الجلب
...
//         dispatch(
//           deleteAppointment({ doctorId: doctor.id, timeSlot: fromSlot }),
//         );
//         dispatch(
//           addAppointment({
//             ...patient,
...
//       gender:
//         p.gender === "female" || p.gender === "Female" ? "female" : "male",
//     });
//   };
...
//                   {DAY_LABELS[doctor.day] || doctor.day} ({displayFormattedDate}
//                   ) • الأسبوع {selectedWeek + 1}
//                 </p>
//               </div>
//               <button
//                 onClick={onClose}
...
//                   const isHasBooked =
//                     status === "has booked" || status === "booked";
//                   const isWaiting =
...
//                   // ✅ هل هذه الخانة هي التي يتم إضافة حجز جديد إليها حالياً؟
//                   const isThisSlotPending = pendingNewSlot === slot;
...
//                       draggable={booking && booking.status === "has booked"}
//                       onDragStart={(e) =>
...
//                             <button
//                               onClick={() =>
//                                 deleteAppointmentServerMutation.mutate(
//                                   booking.uuid,
//                                   {
//                                     onSuccess: () =>
//                                       queryClient.invalidateQueries({
...
// import { db } from "../../../firebase"; // ✅ تأكد من تطابق المسار مع تهيئة Firebase في المشروع
...
//   // 🚀 دالة مساعدة لإضافة أو تحديث موعد في الكاش حسب uuid (Realtime)
...
//   // 🚀 دالة مساعدة لحذف موعد من الكاش حسب uuid (Realtime)
...
//   // 🚀 الاستماع اللحظي (Realtime) لتغييرات مواعيد هذا الطبيب من Firestore
//   // يتم تحديث كاش React Query مباشرة دون أي إعادة جلب كامل (invalidate/refetch)
//   useEffect(() => {
//     if (!doctor?.id) return;
...
//     // 🗑 تنظيف الاشتراك عند الخروج من الشاشة أو تغيّر الطبيب/التاريخ
//     return () => {
//       unsubscribe();
//     };
...
//       if (patient.status !== "has booked" && patient.status !== "has changed") {
//         dispatch(
//           showSnackbar({
//             message: "لا يمكن تحريك موعد في حالة انتظار أو مدفوع",
//             variant: "error",
